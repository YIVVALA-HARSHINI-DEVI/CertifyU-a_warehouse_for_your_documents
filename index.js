...
//   const bryear = req.query.bryear;
//   const StudentsDb = db.collection('Students'); 
//   const details = StudentsDb.doc(regd); 
//   details.set({
//     name: name,
//     regd:regd,
//     email: email,
...
app.post('/studentSignup', [
  ...
  const {name,regd,bryear,email,pwd}= req.body;
  if(!errors1.isEmpty()){
...
app.get('/dateSubmit', (req, res) => {
  ...
  res.render("dateSubmit",{month:month,year:year});
})
app.get('/fac_dateSubmit', (req, res) => {
  const month = req.query.mon;
  const year = req.query.year;
  res.render("fac_dateSubmit",{month:month,year:year});
})
